...
export default {
  ...
  methods:{
    ...
    updateCheckInput(){
      ...
      this.selectionCheck(this.temp_check_patient_id);
    ...
    getCheckEmployee(deptment_id){
      this.$http.get("http://localhost:8082/check/getCheckPatientEmp?deptment_id="+deptment_id).then(
          (res)=>{
    ...
    selectionCheckInput(index,row){
      this.temp_check_patient_id = row.id;
      this.selectionCheck(row.id);
    },
    selectionCheck(id){
      this.$http.get("http://localhost:8082/check/selectionCheckInput?id="+id).then(
          (res)=>{
